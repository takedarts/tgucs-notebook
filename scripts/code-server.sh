#!/bin/sh
# This is a script to install code-server based on the official install script.
# A code is added to register the card to JupyterLab's Launcher.
# by Atsushi Takeda: https://github.com/takedarts/tgucs-notebook
...
register_to_jupyterlab() {
    if grep -q "/tmp/code-server" /etc/jupyter/jupyter_server_config.py; then
        echo "code-server is already registered to JupyterLab."
        return
    fi

    tee --append /etc/jupyter/jupyter_server_config.py > /dev/null << EOF

# Register code-server
if not isinstance(c.ServerProxy.servers, dict):
    c.ServerProxy.servers = {}

c.ServerProxy.servers["code"] = {
    "command": [
        "code-server",
        "--disable-telemetry",
        "--auth", "none",
    ...
    "launcher_entry": {
        "enabled": True,
        "icon_path": "/usr/lib/code-server/src/browser/media/favicon.svg",
        "title": "Code Server (vscode)",
    },
